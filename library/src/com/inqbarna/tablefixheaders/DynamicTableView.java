...
public class DynamicTableView extends ViewGroup {
    ...
    private boolean addRight(int itemRow, int addedRow) {
        ...
        if (column < columnCount && widths[addedRow][column] != 0) {
            //column[0] is for headers so we need to use [column + 1] to get row data
            View view = makeView(addedRow - 1, column, widths[addedRow][column + 1], heights[addedRow]);
            list.add(view);
            return true;
        }
    ...
    private void addVerticalItem(int row, int index) {
        ...
        for (int j = leftColumnIndex[row + 1]; j < columnCount && left < width; j++) {
            ...
            view = makeView(row, j, widths[row + 1][j + 1], heights[row + 1]);
            list.add(view);
        }
        ...
        if (needRelayout || changed) {
            ...
            if (adapter != null) {
                ...
                for (int i = firstRow; i < rowCount && top < height; i++) {
                    ...
                    final View view = makeAndSetup(i, -1, 0, top, widths[i + 1][0], bottom);
    ...
    private void repositionViews() {
        ...
        for (View view : columnViewList) {
            ...
            view.layout(0, top, widths[i][0], bottom);
            top = bottom;
        }
