...
public class TableFixHeaders extends ViewGroup {
    ...
    public void scrollBy(int x, int y) {
        ...
        if (scrollX == 0) {
            ...
            //log("getFilledWidth() " + getFilledHeight() + " width" + width);
    ...
    private void addRight() {
        ...
        int column = firstColumn + size;
        addLeftOrRight(column, size);
    }
    ...
    private int scrollBounds(int desiredScroll, int firstCell, int sizes[], int viewSize) {
        if (desiredScroll == 0) {
            ...
            log("scrollBounds BEFORE " + desiredScroll);
            desiredScroll = Math.max(desiredScroll, -sumArray(sizes, 1, firstCell));
            log("scrollBounds AFTER " + desiredScroll);
        } else {
